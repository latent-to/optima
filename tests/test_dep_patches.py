...
def test_dependency_prebuild_compiles_without_loading(tmp_path, monkeypatch):
    ...
    class FakeSpec:
        def __init__(self, name):
            ...
            self.jit_library_path = (
                ...
                / f"cached_ops/{name}/{name}.so"
            )
            specs.append(self)

        def build(self, *, verbose):
            calls.append(("build", verbose, os.environ["FLASHINFER_CUDA_ARCH_LIST"]))
    ...
    generator_module = types.SimpleNamespace(
        gen_cutlass_fused_moe_sm100_module=lambda use_fast_build=False: FakeSpec("fused_moe_100"),
        gen_cutlass_fused_moe_sm103_module=lambda use_fast_build=False: FakeSpec("fused_moe_103"),
    )
    ...
    # One build per declared architecture, each under its own arch list.
    assert calls == [("build", False, "10.0"), ("build", False, "10.3")]
